...
import { getDashboardHref, getNavItemsForRole, Role } from '@/config/navigation';
...
/* ─── Bottom-tab config: 3-4 primary links per role ─── */
const BOTTOM_TAB_HREFS: Record<string, string[]> = {
  student:       ['/student/dashboard', '/courses', '/quizzes',              '/profile'],
  teacher:       ['/teacher/dashboard', '/courses', '/admin/leaderboard',    '/profile'],
  administrator: ['/admin/dashboard',   '/courses', '/admin/classrooms',     '/profile'],
  superadmin:    ['/superadmin/dashboard', '/superadmin/analytics', '/superadmin/logs', '/profile'],
};
...
export function Navbar() {
  ...
  /* RBAC-filtered nav items for this role */
  const allowedItems = userRole ? getNavItemsForRole(userRole) : [];

  /* Bottom-tab items — pick from allowedItems to ensure RBAC consistency */
  const bottomTabHrefs = BOTTOM_TAB_HREFS[userRole] ?? [];
  const bottomTabItems = bottomTabHrefs
    .map(href => allowedItems.find(i => i.href === href))
    .filter(Boolean) as typeof allowedItems;

  /* No special alias resolution needed — all hrefs are direct page routes */
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4">
          ...
          {isAuthenticated && (() => {
            /* Group items by their group label for section headers */
            const groups: Record<string, typeof allowedItems> = {};
            ...
            return Object.entries(groups).map(([groupLabel, items]) => (
              <div key={groupLabel} className="mb-3">
                ...
                <p className="px-2 pb-1.5 pt-1 font-mono text-[9px] tracking-[0.2em] uppercase text-muted-foreground/60 select-none">
                  {groupLabel}
                </p>
                <div className="space-y-0.5">
                  {items.map((item, index) => {
                    ...
                    return (
                      <Link
                        key={`${groupLabel}-${item.title}`}
                        href={href}
                        onClick={() => setIsMenuOpen(false)}
                        style={{ animationDelay: isMenuOpen ? `${index * 35}ms` : '0ms' }}
                      ...
                      >
                        ...
                        <span className={`font-mono text-[11px] font-bold uppercase tracking-wider transition-colors ${
