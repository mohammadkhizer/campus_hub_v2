...
function SuperAdminContent() {
  ...
  /**
   * BUG FIX: Previously used `defaultValue` (uncontrolled) which doesn't
   * reflect re-fetched data. Now we optimistically update local state so
   * the Select re-renders correctly with the new role immediately.
   */
  const handleRoleChange = async (userId: string, newRole: string) => {
    // Prevent assigning superadmin to themselves
    ...
    // Optimistically update local state so Select reflects change instantly
    setRecentUsers(prev =>
      ...
      <main className="container mx-auto px-6 py-10 max-w-7xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-8 animate-fade-up delay-150">
          {[
            ...
            <div key={i} className={`stat-card border ${item.bg} animate-fade-up delay-${[150,225,300,400][i]}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-up delay-300">
          ...
          {/* User Identity Registry — 2/3 width */}
            ...
            <div className="px-6 py-5 border-b border-border flex items-center justify-between"
              ...
              <div>
                ...
                <p className="font-mono text-xs text-blue-100/70 mt-0.5">
                  Manage global roles and monitor account integrity. Superadmin can assign any role.
                </p>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-border">
                  {recentUsers.map((user: any) => (
                    <tr key={user._id} className="hover:bg-neutral-surface/60 transition-colors">
                      ...
                      {/* Role changer — CONTROLLED (value not defaultValue) */}
                          ...
                          {roleUpdating === user._id ? (
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                {/* ✅ FIX: Always show superadmin option — superadmin CAN grant this role */}
                  ...
                  {recentUsers.length === 0 && (
                    <tr>
                      <td colSpan={4} className="text-center py-12 font-mono text-sm text-muted-foreground">
                        No users found.
                      </td>
                    </tr>
                  )}
